...
_CAPTION_PROMPT = (
    'In 3-4 sentences, capture the core essence, themes, emotional tone, and key ideas of this '
    'content. Focus on what makes it distinctive — be specific and concentrated, not generic.'
)


class GeminiVisionAdapter(AbstractLLMAdapter):
    """
    Universal adapter: captions all content with Gemini 2.5 Flash, then embeds with OpenAI.
    Requires: pip install semantic-tagger[gemini,openai]

    Pipeline (all content types):
        1. Gemini 2.5 Flash generates a concise 3-4 sentence essence caption
        2. OpenAI text-embedding-3-small embeds the caption
        3. Dot product against vocab embeddings (also OpenAI) produces sparse scores

    Captioning normalizes across content length — long posts are compressed to their
    essence, short posts are expanded with interpretation. The caption is returned in
    ScoredOutput.caption so callers can persist it alongside the vector.
    ...
    async def _caption(self, content: ContentItem, gemini_client) -> str:
        """Generate a concise essence caption for any content type."""
        from google.genai import types as gtypes

        if isinstance(content, TextContent):
            resp = await gemini_client.aio.models.generate_content(
                model=self._vision_model,
                contents=[f'{_CAPTION_PROMPT}\n\nContent:\n{content.body}'],
            )
        ...
        elif isinstance(content, (ImageContent, VideoContent, AudioContent)):
            if content.data:  # type: ignore[union-attr]
                media_part = gtypes.Part.from_bytes(data=content.data, mime_type=content.media_type)  # type: ignore[union-attr]
            elif content.url:  # type: ignore[union-attr]
                media_part = gtypes.Part.from_uri(uri=content.url, mime_type=content.media_type)  # type: ignore[union-attr]
            else:
                raise ValueError(f'{type(content).__name__} requires data or url')
            resp = await gemini_client.aio.models.generate_content(
                model=self._vision_model,
                contents=[media_part, _CAPTION_PROMPT],
            )
        else:
    ...
    async def rank(self, content: ContentItem, vocabulary: list[str]) -> ScoredOutput:
        try:
            ...
            raise ImportError(
                'google-genai package is required for GeminiVisionAdapter. '
        ...
        try:
            ...
            caption = await self._caption(content, gemini_client)
            logger.info(
                'GeminiVisionAdapter caption (%s, %d chars): %s…',
                content_type, len(caption), caption[:120],
            )

            input_resp = await openai_client.embeddings.create(model=self._embed_model, input=[caption])
        ...
        return ScoredOutput(scores=scores, content_type=content_type, caption=caption)  # type: ignore[arg-type]
